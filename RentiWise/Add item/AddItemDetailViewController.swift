...
class AddItemDetailViewController: UIViewController {
    ...
    // Make this fileprivate so InlineDropdownController (in the same file) can access it
    ...
    private func showDropdown(for kind: DropdownKind, from sourceRow: UIView?) {
        ...
        // If a dropdown is already visible:
        if let dc = dropdownController {
            if dc.kind == kind {
                // Toggle (close) if tapping the same row again
                dc.dismiss(animated: true)
                dropdownController = nil
                return
            } else {
                // Close existing and open the new one
                dc.dismiss(animated: false)
                dropdownController = nil
            }
        }

        // Build and present a new dropdown
        let dc = InlineDropdownController(kind: kind,
            ...
            case .category:
                ...
                self.selectACategory.textColor = .label // switch from placeholder to normal
            case .condition:
                self.selectCondition.text = selected
                self.selectCondition.textColor = .label // switch from placeholder to normal
            }
...
private final class InlineDropdownController: NSObject, UITableViewDataSource, UITableViewDelegate, UIGestureRecognizerDelegate {
    ...
    private func setupUI() {
        ...
        // Dim background to capture taps outside and dismiss
        backgroundDismissView.backgroundColor = UIColor.black.withAlphaComponent(0.001) // transparent but catches taps
        ...
        tableHeightConstraint = table.heightAnchor.constraint(equalToConstant: 0) // start collapsed
    ...
    // Allow touches inside the table to pass through, but background to dismiss
