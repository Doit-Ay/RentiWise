...
final class ProfileService: ProfileServicing {
    ...
    func fetchCurrentUserProfile() async throws -> UserProfile {
        ...
        do {
            ...
            let phone = row.phone ?? ""
            let upiId = row.upi_id ?? ""

            return UserProfile(id: row.id, fullName: fullName, email: email, phone: phone, upiId: upiId)
        } catch {
            // If DB row not found, at least return auth email so UI shows something
            let email = authUser.email ?? ""
            return UserProfile(id: authUser.id.uuidString, fullName: "", email: email, phone: "", upiId: "")
        }
    }
}
